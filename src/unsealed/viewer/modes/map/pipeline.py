...
import numpy as np
from unsealed.reader.assets.blob import Blob
...
class MapViewerPipeline:
  ...
  def run(self, res: Resource, shader_cache: Optional[Dict] = None) -> MapScene:
    ...
    from unsealed.reader.formats.map.format import SealMapFormat
  ...
  def _load_mdt_blobs(map_res: Resource) -> Dict[str, bytes]:
    ...
    if not mdt_res.exists():
      return {}
    try:
      from unsealed.reader.formats.mdt.decoder import SealMdtDecoder
