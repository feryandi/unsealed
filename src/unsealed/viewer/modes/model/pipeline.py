...
from unsealed.reader.assets import Material
...
class ModelViewerPipeline:
  ...
  def run(self, res: Resource, shader_cache: Optional[Dict] = None) -> ModelScene:
    ...
    from unsealed.reader.formats.base import collect_unknowns

    if res.suffix.lower() == ".act":
      from unsealed.reader.formats.act.format import SealActorFormat

      fmt = SealActorFormat()
    else:
      from unsealed.reader.formats.ms1.format import SealMeshFormat
