...
class AppWorld:
  ...
  def pick_at(self, mx: int, my: int) -> None:
    ...
    hit = self.render.renderer.pick(
      mx, my, self.window.width, self.window.height, view, proj
    )
  ...
  def _start_load(
    ...
    self._bg_error = None
    self._bg_thread = threading.Thread(target=self._run_load, args=(work,), daemon=True)
    self._bg_thread.start()

  def _run_load(self, work: Callable[[], Tuple["ViewerContext", Path]]) -> None:
    try:
