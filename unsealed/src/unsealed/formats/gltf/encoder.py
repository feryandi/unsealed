...
class GltfEncoder:
    ...
    for animation_group_name in animation_groups:
      ...
      for animation in animation_group:
        ...
        for idx, sub in enumerate(
          [animation.transforms, animation.rotations, animation.scales]
        ):
          if len(sub) == 0:
            continue
          ...
          animation_gltf["samplers"].append(
            {"input": a_input, "interpolation": "LINEAR", "output": a_output}
          )
          sampler_idx = len(animation_gltf["samplers"]) - 1
          animation_gltf["channels"].append(
            {
              "sampler": sampler_idx,
              "target": {"node": node_idx, "path": path_names[idx]},
            }
          )
      ...
      if len(joints_accessors) > 0 and len(weights_accessors) > 0:
        ...
        if not is_string_empty(mesh.parent) and mesh.parent is not None:
          try:
            ...
            raise Exception(f"Unable to find parent mesh named: {e}")
  ...
  def __add_accessors_vertices(self, vertices):
    ...
    p_a = self.__add_accessor(b, 5126, len(vertices), "VEC3", min=p_min, max=p_max)
  ...
  def __add_buffer(self, data):
    ...
    if len(bdata) > 0:
      self.gltf["buffers"].append(
        {"byteLength": len(bdata), "uri": "data:application/octet-stream;base64," + b64}
      )
  ...
  def __add_accessors_split_four(self, data, component_type):
    ...
    if component_type == 5126:
      t = "f"
    if t is None:
      raise Exception("Unsupported component type: " + str(component_type))
