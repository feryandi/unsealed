...
class InputSystem:
        ...
        for ev in pygame.event.get():
            if ev.type == pygame.QUIT:
                ...
                if ev.button in (4, 5):
                    ...
                    if ctx is not None:
                        ctx.config.on_scroll(direction, *pygame.mouse.get_pos(), world)
                    ...
                    if ctx is not None:
                        ctx.config.on_mouse_down(ev.button, ev.pos, world)
                ...
                if ctx is not None:
                    ctx.config.on_mouse_up(ev.button, ev.pos, world)
                ...
                if dx == 0 and dy == 0:
                    continue
                if ctx is not None:
                    ctx.config.on_mouse_motion(dx, dy, world)
    ...
    def _on_key(self, key: int, world: "AppWorld") -> None:
        if key == K_ESCAPE:
            ...
            world.scene.context.config.on_key(key, world)
