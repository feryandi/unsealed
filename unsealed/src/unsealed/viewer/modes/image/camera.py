...
class ImageCamera(Camera):
  ...
  def projection_matrix(self, aspect: float, width: int = 0, height: int = 0) -> NDArray:
    ...
    return self.ortho(
      self.pan_x - half_w, self.pan_x + half_w,
      self.pan_y - half_h, self.pan_y + half_h,
      -1.0, 1.0,
    )
  ...
  def zoom_step(self, direction: int, mx: float, my: float, win_w: float, win_h: float) -> None:
  ...
  def reset(self, img_w: int = 0, img_h: int = 0, win_w: int = 0, win_h: int = 0) -> None:
