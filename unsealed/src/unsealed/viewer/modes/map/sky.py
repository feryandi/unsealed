...
class SkyRenderer:
    ...
    def upload(self, sky_meshes: List[ViewerMesh]) -> None:
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, prim.indices.nbytes, prim.indices, GL_STATIC_DRAW)
    ...
    def render(self, view: NDArray, proj: NDArray, time: float = 0.0) -> None:
        """Draw the sky dome.  view/proj are the scene matrices (projection already mirrored)."""
        ...
        # GL_LEQUAL: sky depth = 1.0 passes where no geometry has written (depth = 1.0 clear).
